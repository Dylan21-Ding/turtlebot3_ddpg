...
class CriticNetwork(object):
    ...
    def create_critic_network(self, state_size, action_dim):
        ...
        sinput = Input(shape=[state_size], name="c_state")
        ainput = Input(shape=[action_dim], name='c_action')

        w1 = Dense(512, activation='relu')(sinput)

        h1 = concatenate([w1, ainput])

        h2 = Dense(512, activation='relu')(h1)
        h3 = Dense(512, activation='relu')(h2)
        V = Dense(1, activation='linear')(h3)

        model = Model(input=[sinput, ainput], output=V)
